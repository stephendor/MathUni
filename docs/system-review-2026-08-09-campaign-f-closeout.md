...
| `2026-08-08-coverage-gate-vacuous-when-source-unnumbered` | ACTIONED here - coverage always emits the checked denominator, zero refs is `UNCHECKED`, `--min-refs` can enforce a source expectation, and the drift bundle bounces an unchecked candidate until the caller records `accept-no-checkable-refs` or `block-for-source-repair`. |
...
- focused coverage, drift-bundle, and tag-balance tests include both an undispositioned `UNCHECKED` bounce and an explicit acceptance path;
